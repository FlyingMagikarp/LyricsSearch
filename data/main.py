...
#df_no_es_transl = df_no_es_transl.drop(df[df["Song"].str.contains("spanish", na=False)].index)
#df_no_es_transl = df_no_es_transl.drop(df[df["Song"].str.contains("version", na=False)].index)
...
# write csv rows
tmp = ('\n'.join(final_df.apply(convert_row_to_xml, axis=1)))

# write to file with <add> tag
with open('data/out_utf8.xml', 'w', "utf-8") as f:
    f.write("<add>"+tmp+"</add>")
